  ...
  <div class="row no-gutters">
    <div class="col col-auto">
      <button
        class="header-btn"
        title="Удалить"
        :disabled="noSelectedSchedule || cantDelete"
      ...
      <button
        class="header-btn"
        title="Изменить"
        :disabled="noSelectedSchedule || cantEdit"
        @click="toggleRename">
...
<script>
  ...
  export default {
    ...
    methods: {
      checkRule () {
        if (this.$store.getters.passport.id && this.$store.getters.userGroup.length > 0) {
          this.cantDelete = !RulesChecker.canUserDo('delete', 'schedules', this.$store.getters)
          this.cantEdit = !RulesChecker.canUserDo('edit', 'schedules', this.$store.getters)
          this.cantCreate = !RulesChecker.canUserDo('create', 'schedules', this.$store.getters)
        } else {
    ...
    computed: {
      noSelectedSchedule () {
        return this.$store.getters.targetScheduleId === 0
      }
    }
  }
</script>
