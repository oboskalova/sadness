<template>
  <li class="tree-node" :class="{ 'tree-node--opened': isOpened, 'tree-node--active': isCurrentScene }">
    ...
    <ul class="tree" @click="activateScene">
        <TheSidebarTemplateTreeNode
...
<script>
  ...
  import TheSidebarTemplateTreeNode from './TheSidebarTemplateTreeNode.vue'
  export default {
    name: 'sidebar-template-scene-node',
    props: ['id', 'name', 'nodes', 'current', 'scene'],
    components: {
      TheSidebarTemplateTreeNode
    },
    filters: {
    ...
    methods: {
      ...
      refreshCatalogue: function () {
        this.$store.commit('CHANGE_CURRENT_CONTAINER', this.id)
        this.$store.commit('CHANGE_CURRENT_SCENE', this.id)
